...
const std::vector<uint32_t> SHIFT_AMOUNTS = {7, 12, 17, 22, 5, 9,  14, 20,
...
std::vector<int8_t> computeMD5(const std::vector<int8_t>& message) {
    ...
    for (uint32_t i = 0; i < 8; ++i) {
        padding_bytes[padding_bytes.size() - 8 + i] =
            static_cast<int8_t>(message_length_bits);
    ...
    for (uint32_t i = 0; i < number_blocks; ++i) {
        ...
        for (uint32_t j = 0; j < 64; ++j) {
            ...
            uint32_t temp =
                b + left_rotate(a + f + buffer[buffer_index] + K[j],
...
int main() {
    ...
    for (const std::string& test : tests) {
        std::cout << to_hex_string(computeMD5(to_byte_vector(test)))
                  << " <== \"" + test + "\"" << "\n";
    }
}
